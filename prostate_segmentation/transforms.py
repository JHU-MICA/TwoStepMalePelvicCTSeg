...
def heavy_translation_transform(data_fingerprint,device='cuda:0',spatial_size=(64,64,64),pixdims=(1.7,1.7,1.7)):
    '''
    creates nnUNET transform pipeline for fixed size images with heavy translation
    '''
    ...
    train_transform = mt.Compose(
        [
            ...
            mt.Orientationd(keys=["image", "label"], axcodes="RAS"), # adjust orientation
            mt.Spacingd(keys=["image", "label"], pixdim=pixdims, mode=("bilinear", "nearest")), # change spacing
            RoiCrop(keys=["image", "label"], img_size=[int(x*1.5) for x in spatial_size]), # crop around roi
