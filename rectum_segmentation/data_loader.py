...
def create_datasets(task_name,transforms,random_state=0,cache_dir=None,val_size=0.2,*args,**kwargs):
    ...
    test_imgs = [x for x in data_loc.image if any(substring in x for substring in test_names)]
    test_lbls = [x for x in data_loc.label if any(substring in x for substring in test_names)]
    ...
    '''test_dicts = [
        {'image': img_name,'label': lbl_name}
        for img_name, lbl_name in zip(test_imgs,test_lbls)
    ]'''
